...
→ post via the MCP → screenshot/echo the result.
...
difference → flag anything stale or duplicated → produce a recon summary.
...
go-ahead first unless pre-authorized. Never fabricate a balance or a "reconciled" status.
